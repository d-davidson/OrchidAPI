...
@pytest.fixture
def basic_auth_api():
    api = OrchidAPI(SERVER_URI, auth=(USER, PASSWORD))
    yield api
    api.session.close()
...
def jwt_api(secret, basic_auth_api, encoded_jwt):
    ...
    api = OrchidAPI(SERVER_URI, auth=BearerAuth(encoded_jwt))
    yield api
    api.session.close()
...
def remote_api(jwt_api):
    ...
    api = OrchidAPI(SERVER_URI, auth=BearerAuth(token))
    yield api
    api.session.close()
